...
export class SeedService {
  ...
  async seed(): Promise<void> {
    ...
    const savedUsers: User[] = await this.userRepo.save(
      usersData.map((user: Partial<User>) => this.userRepo.create(user)),
    );
    ...
    const savedGrants: Grant[] = await this.grantRepo.save(
      grantsData.map((grant: Partial<Grant>) => this.grantRepo.create(grant)),
    );
    ...
    await this.grantUserInteractionRepo.save(
      grantUserInteractionsData.map((grantUserInteraction: Partial<GrantUserInteraction>) =>
  ...
  private async cleanDB(): Promise<void> {
    await this.grantUserInteractionRepo.query(
      'TRUNCATE TABLE "grant_user_interaction" RESTART IDENTITY CASCADE;',
    );
